...
class BeneficiaryController extends Controller
{
    ...
    {
        $beneficiaries = Beneficiary::paginate(500);
    ...
    public function store(Request $request)
    {
        ...
        for ($ix; $ix <= $n; $ix++) {
            ...
            $nuevo_plan[] = [
                ...
                round($a + $seguro, 2),
                $saldo_final,
                date('Y-m-15', strtotime($request->input('fecha_inicio') . '+' . $ms . ' month'))
            ];
    ...
    {
        //$vouchers = $beneficiary->vouchers->payments;
        ...
        //return strtotime($beneficiary->fecha_activacion . '+' . $beneficiary->plazo_credito . 'month' );

        $d1 = new DateTime('now');
        $d2 = new DateTime($beneficiary->fecha_activacion);
        //return $d2->format('d-m-Y');   //->diff($d1)->format('%m months');
        $d2->modify('+240 month');

        $dias = $d2->diff($d1)->format('%a');

        $meses_restantes = round($dias / 30.5, 0);

        return view('beneficiaries.show', compact('beneficiary', 'vouchers', 'totalVouchers', 'meses_restantes'  ));
    }
