        ...
        <div class="container grid grid-cols-2 gap-2">
            <div>
                <div class="bg-white shadow-lg rounded-lg p-6 mb-4">
                    ...
                    <div class="flex justify-between mt-4">
                        <div>
                            <p class="text-gray-600">Monto Activado</p>
                            <p class="text-gray-800">{{ 'Bs. ' . number_format($beneficiary->monto_activado, 2) }}</p>
                        </div>
                        <div>
                    ...
                    <div class="flex justify-between mt-4">
                        <div>
                            <p class="text-gray-600">Monto en Cuotas</p>
                    ...
                    <div class="flex justify-between mt-4">
                        <p class=@if ($beneficiary->estado == 'CANCELADO') "text-gray-400" @else "text-gray-600" @endif>Generador
                            de Planes:</p>
                        <p class=@if ($beneficiary->estado == 'CANCELADO') "text-gray-400" @else "text-gray-800" @endif>
                            Permite realizar una nueva serie (plan) de pagos en base a los <b>años especificados</b> y
                            el <b>monto acumulado en cuotas.</b>
                        </p>
                    </div>
                    <div x-data="{ show: false }">
                        <button @if ($beneficiary->monto_activado + $totalVouchers <= 0) style="display: none;" @endif @click="show = !show">
                            ...
                            <form action="{{ route('beneficiary.store') }}" method="post">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" inputmode="decimal" id="capital_inicial"
                                        ...
                                        required value="{{ $beneficiary->monto_activado + $totalVouchers }}"
                                        title="Saldo restante de (Monto Activado menos Monto en Cuotas).">
                                </div>{{--
                                <div class="mb-4">
                                    <label for="n_cuotas" class="block text-gray-700 font-bold mb-2">
                                        Cuotas a pagar:
                                    </label>
                                    <input type="number" id="n_cuotas" name="n_cuotas"
                                        class="appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:bg-white"
                                        required>
                                </div> --}}
                                <div class="mb-4">
                                    ...
                                    <input type="text" id="meses" name="meses" placeholder="Ej: 10"
                                        class="appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:bg-white"
                                        required value="{{ $meses_restantes }}"
                                ...
                                <div class="mb-4">
                                    <div class="block mt-4">
                                        <label for="correlativo" class="flex items-center text-gray-700 font-bold mb-2" title="Desactivado: el n# de cuota empezará desde el 1 para adelante, de lo contrario, desde la ultima cuota correspondiente a los meses restantes.">
                                            <x-checkbox id="correlativo" name="correlativo" />
                                            <span
                                                class="ms-2 text-gray-600 dark:text-gray-400">Mantener correlatividad</span>
                                        </label>
                ...
                <div class="bg-white overflow-x-auto shadow-lg rounded-lg overflow-y-auto">
                    <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50">
                            <tr class="text-md text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800">
                                <th class="px-4 py-4 font-medium text-gray-300 whitespace-nowrap">
                                    N° Cuota
                                </th>
                                                ...
                                                @foreach ($v->payments as $p)
                                                    <x-dropdown-link>
                                                        <span class="cursor-pointer">
                                                            {{ $p->prtdtdesc . ' | Bs. ' . number_format($p->montopago * -1, 2) }}
                                                        </span>
                                                    </x-dropdown-link>
                                                @endforeach
                                            </x-slot>
