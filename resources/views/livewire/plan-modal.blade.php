<div x-data="{ isOpen: @entangle('isOpen') }">
    <x-personal.button @click="isOpen = true" @keydown.escape.window="isOpen = false" variant="primary" size="md"
        iconLeft="fa-solid fa-book">
        Ver {{ $title }}
    </x-personal.button>
    ...
    <div x-show="isOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        <div class="bg-white overflow-hidden rounded-lg transform transition-all sm:max-w-5xl sm:w-full m-4 z-50">
            ...
            <div class="px-2 py-2 sm:p-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                <div class="p-4" id="profile_plans">
                                ...
                                <tr class="bg-gray-200 text-gray-700">
                                    ...
                                    <th class="px-4 py-3 font-semibold text-left">
                                        ...
                                        <span class="block text-green-600 text-sm">
                                            ({{ number_format($beneficiary->getCurrentPlan()->sum('prppgcapi'), 2) }})
                                        </span>
                                    </th>
                                    <th class="px-4 py-3 font-semibold text-left">
                                        <span class="block">Interés</span>
                                        <span class="block text-green-600 text-sm">
                                            ({{ number_format($beneficiary->getCurrentPlan()->sum('prppginte'), 2) }})
                                        </span>
                                    </th>
                                    <th class="px-4 py-3 font-semibold text-left">
                                        <span class="block">Seguro</span>
                                        <span class="block text-green-600 text-sm">
                                            ({{ number_format($beneficiary->getCurrentPlan()->sum('prppgsegu'), 2) }})
                                        </span>
                                    </th>
                                    <th class="px-4 py-3 font-semibold text-left">
                                        <span class="block">Total a Pagar</span>
                                        <span class="block text-green-600 text-sm">
                                            ({{ number_format($beneficiary->getCurrentPlan()->sum('prppgtota'), 2) }})
                                        </span>
                                    </th>
                                    <th class="px-4 py-3 font-semibold text-left">Vencimiento</th>
                                    <th class="px-4 py-3 font-semibold text-left"></th>
                                    <th class="px-4 py-3 font-semibold text-left">Observaciones</th>
                                </tr>
                            </thead>
                                ...
                                @forelse ($plans as $p)
                                    ...
                                    <tr class="border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 text-gray-700">
                                            @if ($p->estado == 'CANCELADO')
                                                Cuota cancelada el: <br /> {{ $p->updated_at }}
                                            @else
